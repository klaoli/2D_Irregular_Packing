 ...
 */
static SlideResult slide(polygon_t& pA, polygon_t::ring_type& rA, polygon_t::ring_type& rB, nfp_t& nfp, const point_t& startTrans, bool inside) {
	...
	while (startAvailable) {
		...
		if (bg::overlaps(pA, rB)) {
			...
			// 输出结果
			std::cout << "The overlapping area is: " << area.val() << std::endl;

			throw std::runtime_error("Internal error: Slide resulted in overlap");
		}
			
		++cnt;
		if (equals(referenceStart, rB.front()) || (inside && boost::geometry::touches(rB.front(), nfp.front()))) {
