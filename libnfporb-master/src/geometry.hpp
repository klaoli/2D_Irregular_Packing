...
static bool equals(const LongDouble& lhs, const LongDouble& rhs);
#ifdef LIBNFP_USE_RATIONAL
...
static std::ostream& operator<<(std::ostream& os, const LongDouble& c) {
	os << toLongDouble(c);
	return os;
}

static std::istream& operator>>(std::istream& is, LongDouble& c) {
	long double val;
	is >> val;
	c.setVal(val);
	return is;
...
static std::ostream& operator<<(std::ostream& os, const point_t& p) {
...
static bool equals(const rational_t& lhs, const rational_t& rhs) {
	return lhs == rhs;
}
#endif

static bool equals(const point_t& lhs, const point_t& rhs) {
	return equals(lhs.x_, rhs.x_) && equals(lhs.y_, rhs.y_);
}

static bool equals(const segment_t& lhs, const segment_t& rhs) {
	return equals(lhs.first, rhs.first) && equals(lhs.second, rhs.second);
}
static std::ostream& operator<<(std::ostream& os, const segment_t& seg) {
	os << "{" << seg.first << "," << seg.second << "}";
	return os;
}

static bool operator<(const segment_t& lhs, const segment_t& rhs) {
...
static point_t normalize(const point_t& vec) {
	point_t norm = vec;
...
static point_t flip(const point_t& vec) {
	point_t flipped = vec;
...
static bool is_parallel(const segment_t& s1, const segment_t& s2) {
...
static Alignment get_alignment(const segment_t& seg, const point_t& pt) {
...
static long double get_inner_angle(const point_t& joint, const point_t& end1, const point_t& end2) {
...
static std::vector<psize_t> find_minimum_x(const polygon_t& p) {
...
static std::vector<psize_t> find_maximum_x(const polygon_t& p) {
	assert(!p.outer().empty());
	std::vector<psize_t> result;
...
static std::vector<psize_t> find_minimum_y(const polygon_t& p) {
	assert(!p.outer().empty());
	std::vector<psize_t> result;
	coord_t min = MAX_COORD;
	auto& po = p.outer();
...
static std::vector<psize_t> find_maximum_y(const polygon_t& p) {
	assert(!p.outer().empty());
	std::vector<psize_t> result;
	coord_t max = MIN_COORD;
	auto& po = p.outer();
...
static psize_t find_point(const polygon_t::ring_type& ring, const point_t& pt) {
 ...
 */
static bool in_nfp(const point_t& pt, const nfp_t& nfp) {
