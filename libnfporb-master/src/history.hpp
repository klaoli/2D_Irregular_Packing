 ...
 */
static off_t find(const History& h, const TranslationVector& tv, std::function<bool(const TranslationVector&, const TranslationVector&)> predicate, const off_t& offset = 0) {
	if(offset < 0)
		return -1;
 ...
 * @return returns The index of the translation vector or -1 if it isn't found.
 */
static off_t find(const History& h, const TranslationVector& tv, const off_t& offset = 0) {
	if(offset < 0)
		return -1;
 ...
 */static size_t count(const History& h, const TranslationVector& tv, std::function<bool(const TranslationVector&, const TranslationVector&)> predicate) {
	size_t cnt = 0;
	off_t offset = 0;
 ...
 * @param tv The translation vector.
 * @return The count of occurrences in the history.
 */static size_t count(const History& h, const TranslationVector& tv) {
