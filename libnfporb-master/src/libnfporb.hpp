 ...
 */
static bool delete_consecutive_repeating_point_patterns(polygon_t::ring_type& ring) {
 ...
 */
static void remove_co_linear(polygon_t::ring_type& r) {
	assert(r.size() > 2);
	psize_t nextI;
	psize_t prevI = 0;
 ...
 */
static void remove_co_linear(polygon_t& p) {
 ...
 */
static nfp_t generate_nfp(polygon_t& pA, polygon_t& pB, const bool checkValidity = true) {
	remove_co_linear(pA);
	remove_co_linear(pB);
